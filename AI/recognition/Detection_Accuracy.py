...
with open('AI\\classified_func\\ensemble_classifier.pkl', 'rb') as f:
    model = pickle.load(f)
    ...
    while cap.isOpened():
        ...
        try:
            ...
            if pose is not None:
                ...
                max_prob = body_language_prob[max_prob_index]
                
                total_predictions += 1
                if body_language_class == actual_class:
                    correct_predictions += 1
                
                accuracy = correct_predictions / total_predictions if total_predictions > 0 else 0
                accuracy_text = f"Accuracy : {accuracy:.2f}"
                cv.putText(frame, accuracy_text, (10, 30), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv.LINE_AA)
